...
// Keep event loop alive no matter what
...
async function bootstrap() {
  ...
  // Create express and listen IMMEDIATELY
  ...
  // Mount NestJS onto the same express app
  console.log('[boot] creating NestJS...');
  const adapter = new ExpressAdapter(expressApp);
  const app = await NestFactory.create<NestExpressApplication>(AppModule, adapter, {
    logger: ['error', 'warn', 'log'],
  });
  console.log('[boot] NestJS created');
